...
document.addEventListener('DOMContentLoaded', () => {
    ...
    var pwaName = "SimeXScan"; //Local Storage Names for PWA
    ...
    function init_template() {
        ...
        var rememberHighlight = localStorage.getItem('Highlight');
        if (rememberHighlight) {
            ...
            if (!document.querySelectorAll('.page-highlight').length) {
                ...
                // localStorage.setItem('Highlight', defaultHighlight[1])
            }
        }
        ...
        var pageBackground = localStorage.getItem('Gradient');
        ...
        if (localStorage.getItem('Theme') == "dark-mode") { for (let i = 0; i < toggleDark.length; i++) { toggleDark[i].checked = "checked" }; document.body.className = 'theme-dark'; }
        if (localStorage.getItem('Theme') == "light-mode") { document.body.className = 'theme-light'; } if (document.body.className == "detect-theme") { setColorScheme(); }
        ...
        */
        ...
        if (isPWA === true) {
            ...
            if (!checkPWA.classList.contains('isPWA')) {
                ...
                var setupTime = localStorage.getItem('-PWA-Timeout-Value');
                if (setupTime == null) {
                    // localStorage.setItem(pwaName + '-PWA-Timeout-Value', now);
                    localStorage.setItem('-PWA-Timeout-Value', now);
                } else if (now - setupTime > hours * 60 * 60 * 1000) {
                    // localStorage.removeItem(pwaName + '-PWA-Prompt')
                    // localStorage.setItem(pwaName + '-PWA-Timeout-Value', now);
                    localStorage.removeItem('-PWA-Prompt')
                    localStorage.setItem('-PWA-Timeout-Value', now);
                }
                ...
                pwaClose.forEach(el => el.addEventListener('click', e => {
                    ...
                    localStorage.setItem('-PWA-Timeout-Value', now);
                    localStorage.setItem('-PWA-Prompt', 'install-rejected');
                ...
                if (pwaWindows.length) {
                    if (isMobile.Android()) {
                        ...
                        if (localStorage.getItem('-PWA-Prompt') != "install-rejected") {
